...
export async function middleware(request: NextRequest) {
  ...
  if (pathname === "/setup/encryption-key") {
  ...
  if (isProtectedRoute || isAuthRoute || isSetupRoute) {
    try {
      const setupStatusResponse = await fetch(new URL("/api/setup/status", request.url), {
        method: "GET",
        headers: {
      ...
      if (setupStatusResponse.ok) {
        ...
        if (setupStatus.missingDestinationEncryptionKey) {
          const url = request.nextUrl.clone();
          url.pathname = "/setup/encryption-key";
...
export const config = {
  matcher: [
    ...
    "/setup/:path*", // Setup routes, including encryption key interstitial
  ],
};
